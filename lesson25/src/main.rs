...
fn main() {
    ...
    // альтернативный сокращенный синтасис для создания структуры (см. Функцию)
    let coffee2: Coffee = make_coffee_2(String::from("Американо"), 8.99, true);
    println!("{},{},{}", coffee2.name, coffee2.price, coffee2.is_hot);
    println!("-------");

    
    // альтернативный сокращенный синтасис для создания структуры
    ...
    let caramel_macchiato: Coffee = Coffee {
        name:latte.name.clone(),  // нету метода трейд копи
        ..latte // ссылаемся на другую структуру и копируем ее поля, за исключение поля name
    };
...
// альтернативный сокращенный синтасис для создания структуры при помощи функции
